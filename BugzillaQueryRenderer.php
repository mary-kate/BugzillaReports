...
class BugzillaQueryRenderer {
	...
	public function renderHTML( $result ) {
		...
		$this->context->debug && $this->context->debug( "Rendering report : rows = $nRows" );
		...
		if (
			...
			foreach ( $this->query->getColumns() as $column ) {
				...
				if ( ( $column == 'summary' ) && ( $nRows > 1 ) ) {
					$name .= " ($nRows tasks)";
					if (
						$this->context->bzserver &&
						$this->query->get( 'bzurl' ) == 'show' &&
						substr(
							$this->query->bzURL,
							strlen( $this->query->bzURL ) -
		...
		while ( $line = $this->connector->fetch( $result ) ) {
			...
			if ( $doGrouping ) {
				...
				foreach ( $groups as $group ) {
					...
					$newGroupValue = $this->query->formatForHeading(
						$line[$this->query->mapField( $group )],
						$group
					);
			...
			if ( $count > $localMaxRows ) {
				if ( $localMaxRows > $this->query->get( 'maxrows' ) ) {
					$this->context->warn(
						"Report truncated - count greater than max allowed " .
							"$count > $localMaxRows"
					);
				}
				break;
			...
			if ( $line['id'] != $currentId ) {
				...
				if ( $this->query->get( 'bar' ) ) {
					if ( $count > $localMaxRowsForBarChart ) {
						$this->context->warn(
							"Bar count greater than max allowed $count > $localMaxRowsForBarChart"
						);
						break;
					}
				...
				if ( !$this->query->get( 'hide' ) ) {
					if ( $this->query->get( 'format' ) == 'list' || $this->query->get( 'format' ) == 'inline' ) {
						if ( $this->query->get( 'format' ) == 'list' ) {
							...
							$this->output .= "<td class=\"bz_list\" colspan=\"{$this->numberOfMainRowColumns}\">";
						} else {
							$this->output .= '<span>';
							if ( $count > 1 ) {
								$this->output .= ', ';
						...
						if ( array_key_exists( 'to', $this->query->getColumns() ) ) {
							$this->output .=
								'[' . $this->query->format( $line[$this->query->mapField( 'to' )], 'to', '' ) . '] ';
						}
						if ( array_key_exists( 'deadline', $this->query->getColumns() ) ) {
							$deadline = $line[$this->query->mapField( 'deadline' )];
							if ( $deadline ) {
								$this->output .= '(' . $this->query->format( $deadline, 'deadline', '' ) . ') ';
							}
						}
						$this->output .= $this->query->format(
							$this->query->getDBValue( $line, $this->query->mapField( 'summary' ) ),
							'summary',
							''
						) . ' (#' . $this->query->format( $line[$this->query->mapField( 'id' )], 'id', '' ) . ')';
						if ( $this->query->get( 'format' ) == 'list' ) {
							$this->output .= '</td></tr>';
						} else {
							$this->output .= '</span>';
								...
								if ( $firstcolumn ) {
									...
									if ( $this->query->get( 'lastcomment' ) || $this->query->get( 'keywords' ) ) {
										...
										if ( $this->query->isRequired( 'keywords' ) ) {
											$keywords = $this->query->format(
												$line[$this->query->mapField( 'keywords' )],
												'keywords',
												''
											);
										}
										if ( strlen( $lastcomment ) > 0 || strlen( $keywords ) > 0 ) {
											...
											$this->output .= '<span class="bug_id">' . $line['id'] . "</span>\n" .
	...
	private function renderDetailsRow( $array, $line, $prepend ) {
		...
		foreach ( array_keys( $array ) as $column ) {
			...
			if ( $line[$column] ) {
				...
				switch ( $array[$column] ) {
					case 'title':
						$title .= $this->query->format(
							$this->query->getDBValue( $line, $dbColumn ),
							$column,
							$valueTitle
						) . ' ';
						break;
					case 'extra':
						$extra .= $this->query->format(
							$this->query->getDBValue( $line, $dbColumn ),
							$column,
							$valueTitle
						) . ' ';
	...
	private function renderTotal( $arrayOfTotals ) {
		...
		foreach ( $this->query->getColumns() as $column ) {
			...
			if ( array_key_exists( $column, $arrayOfTotals ) ) {
				...
				$this->context->debug &&
					$this->context->debug( "Writing $column total = " . $arrayOfTotals[$column] );
	...
	private function renderBar( $barArray ) {
		...
		$this->output .= "<tr class=\"bz_bar $classColourCode\"><td colspan=\"" . ( $this->numberOfMainRowColumns - 1 ) . "\">";
		$i = 0;
		$iNonZero = 0;
		sort( $arrayKeys );
		$rowTotal = 0;
		...
		foreach ( $arrayKeys as $key ) {
			...
			if ( $count > 0 ) {
				...
				$this->output .= "\"$widthString class=\"$class\"><div>" .
					"$content</div></div>";
			}
		}
	...
	public function renderNoResultsHTML() {
		if ( $this->query->get( 'noresultsmessage' ) ) {
			return '<div class="bz_noresults">' . $this->query->get( 'noresultsmessage' ) . '</div>';
		} else {
			return '';
		}
	}
	...
	private function getBarArray() {
		...
		if ( array_key_exists( $this->query->get( 'bar' ), $this->query->fieldValues ) ) {
			...
			$this->context->debug &&
				$this->context->debug(
					'Initialised bar keys ('. $this->query->get( 'bar' ) . '): ' .
